...
begin_end_block_function :
    T_OPEN_B block block_b_end_function
         ;
block_b_end_function :
{!_input.LT(2).getText().equalsIgnoreCase("TRANSACTION")}? block_return T_CLOSE_B;

//return : T_RETURN expr_cpp;
write_stmt :
        ident T_OPEN_P (T_ADD? ident)*  T_CLOSE_P  T_SEMICOLON
       | T_WRITE T_OPEN_P (T_ADD? ident)*  T_CLOSE_P  T_SEMICOLON
;
...
create_table_stmt :
     ...
     ;
create_external_table_stmt:
       T_CREATE T_EXTERNAL T_TABLE table_name create_table_preoptions? create_external_table_definition
    ;
...
create_table_columns_item :
      dtype column_name  /*dtype_len? dtype_attr* create_table_column_inline_cons**/
...
create_function_stmt : ///here edit
      T_CREATE  T_FUNCTION ident create_routine_params? create_function_return (T_AS | T_IS)? declare_block_inplace? single_block_stmt
    ;
create_function_return :
       (T_RETURN | T_RETURNS) dtype dtype_len?
     ;
create_package_stmt :
       T_CREATE T_PACKAGE ident (T_AS | T_IS) package_spec T_END (ident T_SEMICOLON)?
    ;
package_spec :
...
package_spec_item :
    ...
    ;
arrays:  dtype ident T_OPEN_SB T_CLOSE_SB
    | dtype ident T_OPEN_SB ident T_CLOSE_SB
    |dtype ident T_EQUAL ident T_OPEN_SB ident T_CLOSE_SB T_SEMICOLON


    ;

    //////
//init_expression_Secluded :
 //init_array T_SEMICOLON |  assign_array T_SEMICOLON|init_assign T_SEMICOLON|  many_assign T_SEMICOLON
 //;
 //init_array : typedef_functiom_stmt expr_name T_OPEN_SB int_number T_CLOSE_SB (T_EQUAL T_OPEN_B  expr_atom  (T_COMMA expr_atom)*   T_CLOSE_B)?

  //;

  //assign_array : expr_name T_OPEN_SB int_number T_CLOSE_SB  T_EQUAL   expr_cpp   ;
...
cpp_stmt :
        init_expression_cpp_stmt T_SEMICOLON
        |assignment_cpp T_SEMICOLON
        |for_stmt_cpp
        |if_else_stmt_cpp
        |arrays
        |operators
     ;
      ...
      set_value:
        T_EQUAL (expr_cpp | stmt);

parameters :
      parameter (T_COMMA parameter)*;

parameter:
//       init_expression_cpp_stmt ( T_COMMA init_expression_cpp_stmt )*
      dtype ident (set_value)?
 ;
  ...
  create_function_cpp:
   pre_creation T_OPEN_P parameters? T_CLOSE_P  begin_end_block_function
  |T_VOID variable_name T_OPEN_P parameters? T_CLOSE_P begin_end_block


  ;

     assignment_cpp :
        ident T_EQUAL expr_cpp
     ;

     operators :
        variable_name T_ADD T_ADD
        |variable_name T_SUB T_SUB
        |variable_name T_ADD T_EQUAL variable_name
        |variable_name T_SUB T_EQUAL variable_name
        |variable_name T_MUL T_EQUAL variable_name
        |variable_name T_DIV T_EQUAL variable_name
        ;
        ...
        for_stmt_cpp :
         ...
         for_header :
         T_OPEN_P
         (for_begin_condition (T_COMMA for_begin_condition)*)? T_SEMICOLON
         (variable_name compares ident ((T_COMMA|T_AND_cpp|T_PIPE) variable_name compares ident T_COUNT)*)? T_SEMICOLON ( operators (T_COMMA operators)*)?
         T_CLOSE_P
         ;

         for_begin_condition :
         (dtype ident (set_value)?) | assignment_cpp ;
         ...
         if_stmt_c_plus_plus:
                 T_IF T_OPEN_P bool_expr_c_plus_plus  T_CLOSE_P
                  ;

         if_structure_c_plus_plus :
           if_stmt_c_plus_plus  begin_end_block
           ;

         else_if_structure_c_plus_plus : T_ELSE if_structure_c_plus_plus ;
          else_structure_c_plus_plus : T_ELSE ( begin_end_block | stmt );
          if_else_stmt_cpp : if_structure_c_plus_plus (else_if_structure_c_plus_plus)*  (else_structure_c_plus_plus)? ;
         ...
         bool_expr_c_plus_plus :                               // Boolean condition
                 T_NEGATIV? T_OPEN_P bool_expr_c_plus_plus T_CLOSE_P
               | bool_expr_c_plus_plus bool_expr_logical_operator_c_plus bool_expr_c_plus_plus
               | bool_expr_atom_c_plus
               | T_NEGATIV? bool_expr_atom_c_plus
               ;
         ...
         bool_expr_logical_operator_c_plus :
          ...
          bool_expr_binary_c_plus :
                 expr_cpp bool_expr_binary_operator_c_plus expr_cpp  // if you find problem fix it
                ;
           ...
           bool_expr_atom_c_plus :
            ...
            expr_cpp :
                  T_OPEN_P expr_cpp T_CLOSE_P #priority
                 | expr_cpp interval_item  #un
                 | expr_cpp T_MUL expr_cpp #mult
                 | expr_cpp T_DIV expr_cpp #div
                 | expr_cpp T_ADD expr_cpp #add
                 | expr_cpp T_SUB expr_cpp #sub
                 | expr_case                #case
                 | expr_cursor_attribute    #cursor
                 | expr_agg_window_func     #agg
                 | expr_spec_func           #spec
                 | expr_func                #func
                 | expr_atom                #atom
                 | L_NUMBER+                #expr_number
                 ;

//     ================================================================

create_package_body_stmt :
     T_CREATE T_PACKAGE T_BODY ident (T_AS | T_IS) package_body T_END (ident T_SEMICOLON)?
    ;
package_body :
...
package_body_item :
    ...
    ;
create_procedure_stmt :
      T_CREATE  (T_PROCEDURE | T_PROC) ident create_routine_params? create_routine_options? (T_AS | T_IS)? declare_block_inplace? label? proc_block (ident T_SEMICOLON)?
    ;
...
bool_expr_multi_in :
    ...
    ;
bool_expr_binary :
       bool_expr_binary_operator   // we add this grammer to stop number equal to numbe
      | expr bool_expr_binary_operator expr  // if you find problem fix it
     ;
...
bool_expr_binary_operator :
      ident T_EQUAL expr| expr T_EQUAL ident
     | T_EQUAL2
     | T_NOTEQUAL
     | T_NOTEQUAL2
     | T_LESS
...
expr_cursor_attribute :
    ...
    ;
expr_agg_window_func :
       T_AVG T_OPEN_P expr_func_all_distinct? agg_param T_CLOSE_P expr_func_over_clause?
     | T_COUNT T_OPEN_P ((expr_func_all_distinct? agg_param) | '*') T_CLOSE_P expr_func_over_clause?
     | T_COUNT_BIG T_OPEN_P ((expr_func_all_distinct? agg_param) | '*') T_CLOSE_P expr_func_over_clause?
     ...
     | T_MAX T_OPEN_P expr_func_all_distinct? agg_param T_CLOSE_P expr_func_over_clause?
     | T_MIN T_OPEN_P expr_func_all_distinct? agg_param T_CLOSE_P expr_func_over_clause?
     | T_RANK T_OPEN_P T_CLOSE_P expr_func_over_clause
     | T_ROW_NUMBER T_OPEN_P T_CLOSE_P expr_func_over_clause
     | T_STDEV T_OPEN_P expr_func_all_distinct? agg_param T_CLOSE_P expr_func_over_clause?
...
timestamp_literal :                       // TIMESTAMP 'YYYY-MM-DD HH:MI:SS.FFF' literal
	 ...
	 ident :
	 string
       |(L_ID | non_reserved_words) ('.' (L_ID | non_reserved_words))* //ident?  //let it back can you ?
